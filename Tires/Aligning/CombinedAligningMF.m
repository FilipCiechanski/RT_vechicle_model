function [Mz,t] = CombinedAligningMF(CoeffAlign,SFAlign,CoeffLong, SFLong,alpha, kappa,Fz,gamma, pressure, CoeffLat,SFLat)

% Pre-allocate outputs with the same size and type as alpha
Mz = zeros(size(alpha), 'like', alpha);
t = zeros(size(alpha), 'like', alpha);
...
alpha_req = sqrt(alpha_r.^2+(Kx./Kya).^2.*kappa^2).*sign(alpha_r);
alpha_teq = sqrt(alpha_t.^2+(Kx./Kya).^2.*kappa^2).*sign(alpha_t);

s = R0.*(SSZ1 + SSZ2.*(Fy/Fz0)+(SSZ3 + SSZ4 .* dFz).*gamma_z).* LS;

Mz_r = Dr .* cos(Cr .* atan(Br.*alpha_req)) .* cos(alpha);
